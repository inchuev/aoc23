...
func getSeeds(l string) chan int64 {
	seedCh := make(chan int64, 10000)
	go func() {
		seedsRange := fromStrArr(strings.Split(strings.Split(l, ":")[1], " "))
		for i := 0; i < len(seedsRange)-1; i += 2 {
			for j := seedsRange[i]; j < seedsRange[i]+seedsRange[i+1]; j++ {
				seedCh <- j
			}
		}
		close(seedCh)
...
func findLocation(rm [][]mapping, seeds <-chan int64) int64 {
